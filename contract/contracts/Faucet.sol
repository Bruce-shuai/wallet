...
contract Faucet {
  ...
  constructor(address _tokenContractAddress, uint256 _amountEachTime) {
    tokenContract = IERC20(_tokenContractAddress); // 这个是啥用法
  ...
  function withdraw()  external {
    if (recivedRecord[msg.sender] > 0) {
      require(recivedRecord[msg.sender] - block.timestamp >= 1 days, "You can only request token once every 24 hours");
    }
    require(
